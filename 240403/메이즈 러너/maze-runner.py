...
def turn_maze():
    ...
    # print(max(max(exit[0], people_info[min_num][0]) - min_dst, 0), max(max(exit[1], people_info[min_num][1]) - min_dst, 0), min_dst)
    ...
    for i in range(0, min_dst + 1):
        for j in range(0, min_dst + 1):
            ...
            if exit == [sx + i, sy + j]:
                # print(f"sx{sx} sy{sy} i{i} j{j} min_dst{min_dst}")
