...
def do_laser(nx, ny, vx, vy, visited, path):
    ...
    for i in range(0, 8, 2):
        if ground[(nx + dx[i]) % n][(ny + dy[i]) % m][0] != 0 and not visited[(nx + dx[i]) % n][(ny + dy[i]) % m]:
            path.append([(nx + dx[i]) % n, (ny + dy[i]) % m])
            visited[nx][ny] = True
            do_laser((nx + dx[i]) % n, (ny + dy[i]) % m, vx, vy, visited[:], path[:])
            path.pop(-1)
            visited[nx][ny] = False
